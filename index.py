...
model = Sequential()
model.add(Dense(101,input_shape=(len(train_x[0]),),activation = 'relu'))
...
model.fit(np.array(train_x),np.array(train_y),epochs=180, batch_size =5)
...
import os
import copy
from flask import Flask, render_template,request,send_from_directory,jsonify,redirect,url_for,g
...
def voz():
    ...
    res = get_response(ints,sentimientos)
    tag= chalala(ints,sentimientos)
    paleta=tag
    image_names = os.listdir('./images/'+tag)
    cur = mysql.connection.cursor()
    id_producto=tag
    cur.execute("SELECT * FROM app_voz WHERE id_producto ='"+id_producto+"'")
    datas = cur.fetchall()
    cur.close()
    #datas += image_names
    #datac=jsonify(datas)
    #image_namess= jsonify(image_names)
    
    
    #return resultado

    return render_template("buscar.html", image_names=image_names,res=res,datas=datas,message=message)

@app.route('/<filename>')
def send_image(filename):
    global paleta
    return send_from_directory("images/"+paleta, filename)
